...
- `/refresh-ai-memory` — syncs AI-critical documents (`CLAUDE.md`, `README.md`) against current `.context/` and `package.json`. Touches FACTS only — never structural sections.
...
`/sprint-dev` reads Level 1 + the relevant Level 2 + the story's Level 3 when it starts a ticket. `/product-management` writes Level 2 + Level 3 when it seeds stories. `/refresh-ai-memory` keeps Level 1 in sync with reality.
...
| Refresh AI memory (CLAUDE.md + README.md) | `/refresh-ai-memory` |
...
- **`.context/`** = what the AI KNOWS about the product (generated by `/project-foundation`, kept fresh by `/refresh-ai-memory` + `/business-*` commands).
