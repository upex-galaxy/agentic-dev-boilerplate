...
- **User-level skills installed via `bunx skills add`** (cross-cutting, repo-agnostic): `skill-creator`, `find-skills`, `gh-cli`, `github-actions-docs`, `brainstorming`, `cli-printing-press`, `html-ppt`, plus every CLI-companion skill (`supabase`, `supabase-postgres-best-practices`, `deploy-to-vercel`, `resend-cli`, `bun`, `playwright-cli`). These follow the user across every project.
