...
const SKILL_PURPOSES: Record<string, string> = {
  ...
  'react-hook-form': 'Form state management with react-hook-form.',
  'zod': 'Schema validation with Zod.',
  'shadcn': 'shadcn/ui component library patterns and usage.',
  'tailwind-css-patterns': 'Tailwind CSS utility patterns and design tokens.',
  'typescript-advanced-types': 'Advanced TypeScript type patterns.',
  'accessibility': 'Web accessibility (a11y) best practices and auditing.',
  'seo': 'SEO best practices for Next.js and web apps.',
  'n8n-skills': 'n8n workflow automation MCP toolkit.',
  'emil-design-eng': 'Design engineering patterns from Emil Kowalski.',
  'ui-ux-pro-max': 'UI/UX design intelligence across 10 stacks and 50+ styles.',
  ...
  return {
    ...
    prerequisites: [
      ...
      },
      {
        name: 'node >= 18',
        required: true,
        purpose: 'Used by the npx wrapper that runs create-agentic-dev. Must be v18 or newer.',
      ...
      {
        name: 'an agent harness (Claude Code, OpenCode, or Codex)',
        required: false,
        purpose: 'The scaffolded project ships one instruction body (AGENTS.md) and one skill store (.agents/skills/) that all three harnesses read; install whichever you use. Codex loads the project .codex/ config only in a trusted repository.',
        installHint: 'https://github.com/upex-galaxy/agentic-dev-boilerplate#readme',
      },
    ],
    willInstall: {
    ...
    willConfigure: {
      ...
      envFiles: [
        ...
        { path: 'AGENTS.md', what: 'Already committed. The ONE instruction body every harness reads (OpenCode and Codex natively). Edit this file, never CLAUDE.md.' },
        ...
        { path: '.agents/skills/', what: 'Already committed. The canonical skill store all three harnesses read; community skills installed by `bun run setup` land here too.' },
        { path: '.claude/skills', what: 'Generated by `bun run setup` (via agents:compat): POSIX symlink / Windows junction onto .agents/skills, gitignored. Never edit or commit it.' },
        { path: '.claude/commands/ + .opencode/commands/', what: 'Already committed, then re-rendered from .agents/compatibility/command-aliases.json — thin slash-command wrappers that forward to a skill mode.' },
        { path: '.opencode/plugins/', what: 'Already committed. OpenCode adapter for the shared personality hook in .agents/hooks/ (Claude Code and Codex run the same emitter from .claude/settings.json and .codex/hooks.json).' },
      ...
      postInstallSteps: [
        {
          step: 'agents:compat',
          what: 'Generates the Claude Code surfaces from their sources: the .claude/skills alias onto .agents/skills, the CLAUDE.md shim check, and the slash-command wrappers for Claude Code + OpenCode. Re-run after `bun run up` or in a new worktree.',
          interactive: false,
        },
        {
          step: 'agents:setup',
