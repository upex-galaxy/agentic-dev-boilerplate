...
async function main(): Promise<number> {
  ...
  if (args.version) {
    process.stdout.write(`create-agentic-dev v${VERSION}\n`);
    return 0;
  }

  log.banner('create-agentic-dev — bootstrap');
  ...
  if (args.here) {
    ...
    if (isAgenticDevRepo(cwd, args.templateRepo)) {
      log.success('Existing agentic-dev project detected. Skipping bootstrap.');
      runStageA = false;
    }
    else if (!isDirectoryEmpty(cwd)) {
      throw new CliError(
        'CONFLICT',
        `Current directory is not empty and is not an agentic-dev project: ${cwd}`,
    ...
    if (existsSync(projectDir)) {
      if (isAgenticDevRepo(projectDir, args.templateRepo)) {
        log.success(`Existing agentic-dev project at ${projectDir}. Skipping bootstrap.`);
        runStageA = false;
      }
...
function deriveNameFromPath(p: string): string {
  ...
  return parts[parts.length - 1] ?? 'agentic-dev-app';
}
