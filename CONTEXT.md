...
│   ├── onboarding/                 HTML onboarding artifact
...
> **History note.** Earlier versions of this template used `.prompts/` for workflow content and `.context/guidelines/` for rules. Both are retired — workflow content moved to `.claude/skills/`, and rules now live inside each skill's `references/` folder. The CLI (`bun run up`) v4.1+ no longer syncs the legacy directories.
...
| `.context/_framework/skill-registry.md`           | Compact-rules cache (auto-generated)                                   |
...
```
/agentic-dev-core     → Bootstrap .agents/, scripts, CLAUDE.md (template → real)
...
| **New contributor**       | `/agentic-dev-onboard`; `docs/getting-started.md`                                    |
...
1. **Run `bun run lint:agents` after touching any prompt or skill** — validates every `{{VAR}}` and `{{jira.*}}` reference resolves against config.
...
1. **Don't invoke `/agentic-dev-core` on an already-bootstrapped repo** — it's a one-time scaffolder, not an update tool.
2. **Don't create new files under `.context/` without a generator** — they will drift and rot. Add the generator first.
3. **Don't hand-edit `.agents/jira-fields.json` or `.context/_framework/skill-registry.md`** — both are regenerated by scripts.
