...
- `< 3` → **Deferred** (log in §10, do not include in waves)

### Phase 3 — Dependency mapping

For every Wave 0 and Wave 1 item, trace which downstream features become possible (or required) once it ships. Two directions matter:
...
> **Update frequency**: Re-run when scope shifts, after major architectural decisions, when a Wave 0 / Wave 1 feature ships, or at the start of each quarter / milestone.
```
...
- **How an experienced engineer would scope it** — 3–5 prose bullets covering the smallest credible MVP slice, the most likely scope creep, and the boundary with the next wave

Prose, not code. No payloads, no schemas, no endpoint shapes.

### 4. Wave structure (Wave 0 → Wave 1 → Wave 2)

A short prose summary of each wave: what is in it, what defines "done" for the wave (e.g. "Wave 0 is done when an authenticated user can see an empty dashboard backed by real data"), and the rough size signal (number of features, number of weeks, or "small / medium / large" if estimates are unavailable).
...
This is usually where the most leverage hides — a tight Wave 0 ships the rest faster.
...
- Acceptable stand-in during early waves (mock provider, manual workaround, env-flag stub)

Often the right move is to integrate a service one wave **before** the feature that needs it, so the integration is debugged in isolation.
...
Point is: shipping a Wave 1 feature in isolation is fine for the demo, but the **chain** is what ships value. Spell out the 2–3 chains that matter.
...
- Verify the feature is wired behind a feature flag if it is part of a partial-wave rollout.
...
- Deferred / won't-do features → list here at the end of this section, do not promote them into waves
```

Finish §11 with a short "Deferred / won't-do" subsection listing anything explicitly **excluded** from the MVP plus a one-line reason (e.g. "social login — deferred to Wave 3 after primary auth is hardened"). The point is to make the deferral **visible** so it stops re-entering scope discussions.
...
Each gap should be phrased as a one-line spike: "Spike: choose payment provider (Stripe vs Adyen) before Wave 1 — affects checkout, refund, and webhook design."
...
- In UPDATE mode: show diff summary, wait for explicit confirmation before overwriting. Highlight changes that promote / demote a feature across waves — those are the high-attention deltas.
- Report:
  - Wave 0 features identified: N
  - Wave 1 features identified: N
...
- If any §12 spike is rated as "blocks Wave 0", call it out explicitly in the report — these need a decision before the plan is actionable.
