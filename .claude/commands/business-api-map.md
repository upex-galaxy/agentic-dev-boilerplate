...
**Audience**: developers who need to understand the API contract well enough to implement, modify, or extend features. Keep prose business-first — a new contributor should grasp *why* each journey exists before they touch the code that backs it.
...
The "Why it matters for dev" column answers questions like: *where do new endpoints go?*, *what do I have to mock when I unit-test this?*, *which layer owns the business rule?*. One diagram total for the whole system, not one per journey.
