...
When in doubt, prefer `capture_prompt: true`. Flip to `false` only for auto-generated artifacts (SDD proposal/spec/design/tasks/apply/verify/archive reports, onboarding/state artifacts, skill-registry output).
