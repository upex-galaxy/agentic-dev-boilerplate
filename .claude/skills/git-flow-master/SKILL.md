...
- The boilerplate ships `.agents/project.yaml` with `git_strategy.strategy: null` (a populated-per-project template; the updater freezes it via `bootstrapOnlyPaths`), so a fresh project created from this boilerplate starts with an unset strategy → this offer fires on first real use — by design (template-trap guard).
