...
| `.agents/project.yaml`         | does the file exist + is `active_env` set?                                                                                                                 | Pulls `{{WEB_URL}}`, `{{API_URL}}`, project name, Jira project key. If missing → user must run `/agentic-dev-core` first.            |
