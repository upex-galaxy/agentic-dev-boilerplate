...
**Fases Asincrónicas (iterativas por sprint):** 3. **FASE 3**: Usa prompts de `.prompts/fase-3-specification/` para generar archivos en `PBI/` 4. **FASE 4**: Usa prompts de `.prompts/fase-4-shift-left-testing/` para generar test plans en `PBI/` 5. **FASE 5**: Usa prompts de `.prompts/fase-5-planning/` para generar implementation plans en `PBI/` 6. **FASE 6**: Implementación (usa `guidelines/` como referencia, NO hay prompts) 7. **FASE 7**: Code Review (usa `guidelines/code-standards.md`, NO hay prompts) 8. **FASE 8**: Usa prompts de `.prompts/fase-8-test-automation/` para generar docs de testing en `guidelines/tae/`
