...
Per-epic and per-ticket workspace shared by `/product-management` (backlog + AC refinement) and `/sprint-dev` (story-level dev loop).
...
│   └── feature-impl-plan.md                  Feature-level implementation plan  (/sprint-dev macro)
└── {ticket}/
    ...
    ├── impl-plan.md                          Story implementation plan                   (/sprint-dev Stage 1)
    ├── review.md                             Code-review findings against AC + standards (/sprint-dev Stage 3)
    ├── compliance-matrix.md                  AC-vs-code coverage matrix                  (/sprint-dev Stage 3)
    └── bug-fix.md                            Root-cause + fix plan + regression notes    (/sprint-dev Stage 2 bug-fix)
```
...
3. `/sprint-dev` (per ticket): writes `{ticket}/impl-plan.md` in Stage 1, then `{ticket}/review.md` + `{ticket}/compliance-matrix.md` in Stage 3. Bug-fix branch produces `{ticket}/bug-fix.md`.
...
DEV uses **engram** + **Jira** as canonical sources of cross-session state. `/sprint-dev` rehydrates from both automatically — no in-tree `PROGRESS.md` or `ROADMAP.md` files needed.
